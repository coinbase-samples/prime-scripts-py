...
# must be from the past 3 months
time_offset = datetime.now() - relativedelta(days=60)
start = time_offset.strftime('%Y-%m-%dT%H:%M:%SZ')
end = datetime.now().strftime('%Y-%m-%dT%H:%M:%SZ')

uri = f'https://api.prime.coinbase.com/v1/entities/{ENTITY_ID}/margin_summaries?start_date={start}&end_date={end}'
